...
public class Perspective : Sense
{
    ...
    void DetectAspect()
    {
        ...
        if ((Vector3.Angle(rayDirection, transform.forward)) < FieldOfView)
        {
            ...
            if (Physics.Raycast(transform.position, rayDirection, out hit, ViewDistance))  //Raycast를 ViewDistance만큼 쏜다.
            {
                Aspect aspect = hit.collider.GetComponent<Aspect>();  //태그로 대신할 수 있다.
                if (aspect != null)
                {
                    //Check the aspect
